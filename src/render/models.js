...
export function createHumanoid(scene, {
    ...
    if (weapon === 'sword') {
        ...
        // Портупея идёт от плеча к поясу и ЗАКАНЧИВАЕТСЯ на нём. Прежние
        // 1.24 были перелётом: ремень торчал на 0.12 ниже пояса и на 0.04
        // выше нагрудника — снизу он читался отдельной коричневой палкой,
        // висящей в теле.
        //
        // Толщину тоже увеличили: при 0.05 это была плоская доска, у
        // которой с бока виден только торец.
        const strap = track(MeshBuilder.CreateBox(`${id}_strap`, {
            width: 0.1, height: 0.92, depth: 0.09,
        }, scene));
        strap.parent = torso;
        strap.position.set(0.04, 0.51, 0.13);
    ...
    if (weapon === 'sword') {
        ...
        // Локоть отведён от тела и не заломлен назад. Полный разворот руки
        // пробовал — он делает замах нерешаемым (47 000 вариантов, ни
        // одного с целым хватом и чистым телом), поэтому правка точечная:
        // плечо чуть вперёд по X, остальное как было.
        shoulderR.rotation.x = 0.10;
