...
Полное определение и обоснование — в `GOALS_100.md`. Короткий чек-лист — **все цели закрыты ✅**:
...
- [x] **Аудио:** WebAudio-синтез — боевая тема (арпеджио + бас + шумовой снейр), SFX
      (hit/critical/spell/heal/item/boss/victory/defeat/cancel/evade/ui/levelup);
      переключатели «Звуковые эффекты» и «Музыка» в options; тема стартует в бою и
      останавливается вне боя; победа/поражение озвучены.

## Итог — все 10 целей GOALS_100 закрыты

- Бестиарий: 74 presets (51/51 канонических обычных + 7 канонических боссов) + 52 drop tables.
- Предметы/экипировка: 57 расходников, 53 магазина, 126 экипировки, 13 key-предметов.
- Магия: 138 actions, 42/42 канонических заклинаний, уровни Lv1–5 за SC/MC, 8 Mana Eggs в бою.
- Статусы: confusion и paralysis полность
- Скрипт: 64 NPC-диалога + defeat-реплики всех 19 битов.
- Секреты: Special Stage, армрестлинг Хембла, Carro, Elmo, сундуки в 43 локациях.
- Menu parity: 7 экранов + курсорная навигация + equip preview + Valuables + options.
- Арт: 80 портретов, бэкдропы всех 100 локаций.
- Аудио: WebAudio-боевая тема и 12 SFX.
- Баланс: novice 59.7% / veteran 100% (без регрессий), все аудиты зелёные.

## Полировка (P3, финальный проход)

- Палитры ролей: все новые роли врагов (sand-harasser, gravity-eye, twin-fist, apex-god и др.)
  добавлены в roleAccent (UI) и генератор портретов; сгенерированы 80 портретов с акцентами.
- Сортировка командного меню: новые actions (boom/baBoom/meteorStrike/gadZap/craze/poizn,
  бомбы, свитки, камни стихий, spiderweb…) в actionSortScore.
- FX-палитры: все новые типы действий получили цвета в drawEventFxOverlay.
- Оффенсивные предметы выделены в группу «Offensive items» в командном меню и handbook.
- Босс-фазы: портрет меняет палитру (hue-rotate + saturate) при фазе 2/3 — и в live-бою,
  и в replay viewer.
- Аудио: добавлена спокойная overworld-тема для travel/overworld кампании (переключение
  контекста), боевая тема остаётся для боёв; озвучен исход боя и в песочнице
  (victory/defeat с guard от повторов).
- Регенерированы все аудиты; баланс без изменений.
