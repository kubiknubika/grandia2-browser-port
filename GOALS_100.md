...
Текущее: **64 optional NPC dialogues** (было 30) — покрыты все ключевые города, данжи, комнаты
и late-game локации. Осталось: NPC-реплики в рядовых домах-интерьерах и defeat-реплики боссов.

DoD: **выполнено** — 64 диалога + defeat-реплики боссов для всех 19 битов.
...
Текущее: 6 экранов + **курсорная навигация** — ✅.
- **equip flow с превью статов до/после** — ✅ (в экране «Предметы»).
- **экран Valuables** (Roan's Medal, Book of Sages, медальон рыцаря, дневник инженера…) — ✅.
- options: сброс сохранений кампании и сброс настроек — ✅.

DoD: **выполнено** — `MENU_AUDIT.md` обновлён.

---

## 8. Арт: портреты и бэкдропы до 100% покрытия — M/L

Текущее: **80 портретов** (74 врага + 6 героев), 4 battlefield, 11 region backdrops,
**все 100 локаций** покрыты region-группами + процедурными стилизациями по state-профилю — ✅.
Осталось (опц.): босс-вариации портретов по фазам — P3.

---

## 9. Аудио: музыка и SFX — L (опциональная цель)

В оригинале культовый саундтрек (~40 треков). Для прототипа реализовано:
- **WebAudio/синтез: ✅** боевая тема (минорное арпеджио + бас + шумовой снейр) +
  12 SFX (hit/critical/spell/heal/item/boss/victory/defeat/cancel/evade/ui/levelup).
- **Переключатели в options: ✅** «Звуковые эффекты» и «Музыка», сохраняются в localStorage.
- Тема стартует автоматически в бою и останавливается вне боя; победа/поражение озвучены.
- Опц. развитие: дополнительные темы (город/данж/финал) — P3.
